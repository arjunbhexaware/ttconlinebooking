...
define(["utils","settings"], function (utils,settings) {
    ...
    methods.card = (data) => {
        ...
        for (let i in data.payload) {
			...
			if(data.responseIndex){
				cardBody +=`<img style="border-radius:50%;border:2px solid white;float: left;margin-right: 10px;" width="40" height="40" src='${settings.botAvatar}'/><div class="beforeAfter cardBeforeAfter pmd-card pmd-card-default pmd-z-depth custom-infocard" >`
			}else{
				cardBody +=`<img style="border-radius:50%;float: left;margin-right: 10px;" width="40" height="40" src='avatar/blank.ico'/><div class="pmd-card pmd-card-default pmd-z-depth custom-infocard">`
			}
            cardBody += `
                <!-- Card header -->
            ...
            if (data.buttons && data.payload[i].type == 1) {
                ...
                cardButtons = `<div class="pmd-card-actions" style="font-size:11px !important">`
